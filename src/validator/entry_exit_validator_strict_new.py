...
def condensed_zone_sequence(zone_hist, min_confirm):
    """
    Condense zone history into confirmed zone visits.
    Only zones occupied for at least min_confirm consecutive frames are considered confirmed.
...
def main():
    ...
    print(f"\n{'='*60}")
    print("STRICT WRONG-WAY VIOLATION DETECTOR (Enhanced v2.1)")
    print(f"{'='*60}")
    ...
    for frame_idx, recs in tqdm(frames_tracks, desc="Validating frames"):
        ...
        for r in recs:
            ...
            confirmed_seq, condensed = condensed_zone_sequence(
                tracks_zone_hist[tid], args.min_zone_confirm
            )
